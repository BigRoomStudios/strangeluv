...
* `cdk bootstrap -v`
* `cdk deploy -v`
* Often S3 buckets will be left behind that will cause CloudFormation stacks to fail recovery and other pains in the butt.
  * `aws s3 rb s3://bucket-name --force
    * rb - 'remove bucket'
    * --force lets you delete a bucket with objects in it
* To ensure you've cleaned up everything generated by cdk, if you run `cdk deploy -v` it should tell you that you need to run bootstrap first.
...
* `npm run cdkcleanup`
  ...
  * `aws cloudformation delete-stack --stack-name StrangeluvStaticSite`
* Delete bucket created by `cdk bootstrap`
  * List existing buckets with `aws s3 ls`
  * Copy bucket name — the pattern should be like `cdktoolkit-stagingbucket-xxxxxxxxxx`
  * Run `aws s3 rb s3://cdktoolkit-stagingbucket-xxxxxxxxxxx --force`
